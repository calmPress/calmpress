<?php
/**
 * Network Email delivery Settings Administration Screen.
 *
 * @package calmPress
 */
...
if ( ! is_super_admin() ) {
	wp_die( esc_html__( 'Sorry, you are not allowed to manage email delivery for this network.' ) );
}
...
function name() : void {
	...
	<input
		type="text"
		name="calm_email_transport[from_name]"
...
<div class="wrap">
	...
	<p>
		<?php
			...
			printf(
				esc_html__( 'Configure the default email delivery settings for the network. 
You can test settings related to SMTP connectivity, sender email, and general email delivery in the %s
page before applying them here. Individual sites can use different settings if site-specific email delivery settings are enabled.' ),
				'<a href="' . esc_url( network_admin_url( 'test-email-delivery.php' ) ) . '">' .
