...
/**
 * A DRY to send a reply which indicates that the request sent was in invalid form
 * or data in a json format with a http code 400.
 * 
 * @since calmpress 1.0.0
 */
function send_invalid_request() {
	wp_send_json_error( __( 'Invalid request' ), 400 );
}
...
function wp_ajax_closed_postboxes() {
	...
	if ( is_array( $hidden ) ) {
		...
		$hidden = array_diff( $hidden, array( 'submitdiv', 'manage-menu', 'create-menu' ) );
...
function wp_ajax_query_themes() {
	...
	foreach ( $api->themes as &$theme ) {
		...
		if ( ! is_multisite() && current_user_can( 'edit_theme_options' ) && current_user_can( 'customize' ) ) {
			$customize_url = wp_customize_url( $theme->slug );
...
/**
 * Ajax handler for initiating a webauthn supporting device registration.
 *
 * @since calmPress 1.0.0
 */
function wp_ajax_webauthn_challenge() {

	$user = wp_get_current_user();

	// Reuse the password reset nonce.
	check_ajax_referer( 'reset-password-for-' . $user->ID, 'nonce' );
	...
	try {
		...
		wp_send_json_error( __( 'Can not add more devices.' ), 400 );
	}
}

/**
 * Ajax handler for registering a new webauthn device.
 *
 * @since calmPress 1.0.0
 */
function wp_ajax_webauthn_register_device() {
    ...
    if ( ! isset( $_POST['name'], $_POST['payload'] ) ) {
        send_invalid_request();
    }

	$desc    = wp_unslash( $_POST['name'] );
...
/**
 * Ajax handler for revoking an authenticator credentials.
 *
 * @since calmPress 1.0.0
 */
function wp_ajax_webauthn_revoke() {

	$user = wp_get_current_user();

	if ( ! isset( $_POST['nonce'], $_POST['credential_id'] ) ) {
		send_invalid_request();
	}
	...
	$cred = base64URL_decode( wp_unslash( $_POST['credential_id'] ) );
	...
	try {
		...
		wp_send_json_error( __( 'Failed removing the device.' ), 500 );
	}
}

/**
 * Ajax handler for changing an authenticator description.
 *
 * @since calmPress 1.0.0
 */
function wp_ajax_webauthn_set_description() {

	$user = wp_get_current_user();

	if ( ! isset( $_POST['nonce'], $_POST['credential_id'], $_POST['description'] ) ) {
		send_invalid_request();
	}
	...
	$cred = base64URL_decode( trim( wp_unslash( $_POST['credential_id'] ) ) );
	$description = trim( wp_unslash( $_POST['description'] ) );
