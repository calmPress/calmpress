...
function network_step1( $errors = false ) {
	...
	<p><?php _e( 'Fill in the information below and you&#8217;ll be on your way to creating a network of calmPress sites. Configuration files will be created in the next step.' ); ?></p>
	<?php
	...
	if ( allow_subdomain_install() && allow_subdirectory_install() ) :
		...
		<p><?php _e( 'Please choose whether you would like sites in your calmPress network to use sub-domains or sub-directories.' ); ?>
	...
	if ( $is_www ) :
			...
			<tr>
				...
				<?php
					printf(
						...
						__( 'Because you are using %1$s, the sites in your calmPress network must use sub-directories. Consider using %2$s if you wish to use sub-domains.' ),
						'<code>localhost</code>',
		...
		<?php elseif ( ! allow_subdomain_install() ) : ?>
			<tr>
				<th scope="row"><?php esc_html_e( 'Sub-directory Installation' ); ?></th>
				<td><?php
					_e( 'Because your installation is in a directory, the sites in your calmPress network must use sub-directories.' );
					// Uh oh:
				if ( ! allow_subdirectory_install() ) {
					echo ' <strong>' . __( 'Warning:' ) . ' ' . __( 'The main site in a sub-directory installation will need to use a modified permalink structure, potentially breaking existing links.' ) . '</strong>';
				}
		...
		<?php elseif ( ! allow_subdirectory_install() ) : ?>
			<tr>
				...
				<td><?php _e( 'Because your installation is not new, the sites in your calmPress network must use sub-domains.' );
...
function network_step2( $errors = false ) {
	...
	$home_path         = 0 === strpos( $abspath_fix, $document_root_fix ) ? $document_root_fix . $base : ABSPATH;
...
?>
	<div class="error"><p><strong><?php _e( 'Warning:' ); ?></strong> <?php _e( 'An existing calmPress network was detected.' ); ?></p></div>
...
define( 'BLOG_ID_CURRENT_SITE', 1 );
	...
	<?php
	if ( is_nginx() ) { // Link to Nginx documentation  of trying to generate rules:
...
EOF;
		...
		printf(
			...
			__( 'Add the following to your %1$s file in %2$s, <strong>replacing</strong> other calmPress rules:' ),
			'<code>.htaccess</code>',
			'<code>' . $home_path . '</code>'
		);
		echo '</p>';
		...
		<?php
	} // End Nginx/Apache code branches.
