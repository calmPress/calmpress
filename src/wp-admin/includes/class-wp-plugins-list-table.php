...
class WP_Plugins_List_Table extends WP_List_Table {
	...
	public function __construct( $args = array() ) {
		...
		$allowed_statuses = array( 'active', 'inactive', 'recently_activated', 'upgrade', 'mustuse', 'dropins', 'search', 'core' );
	...
	public function prepare_items() {
		...
		if ( empty( $plugins[ $status ] ) && ! in_array( $status, array( 'all', 'search', 'core' ), true ) ) {
			$status = 'all';
		}
	...
	protected function extra_tablenav( $which ) {
		...
		if ( ! in_array( $status, array( 'recently_activated', 'mustuse', 'dropins', 'core' ), true ) ) {
			return;
		}
	...
	public function single_row( $item ) {
		...
		if ( $screen->in_admin( 'network' ) ) {
			 ...
			 *                              'recently_activated', 'upgrade', 'mustuse', 'dropins', 'core',
			 *                              and 'search'.
			 */
			 ...
			 * @param string   $context     The plugin context. By default this can include 'all',
			 *                              'active', 'inactive', 'recently_activated', 'upgrade',
			 *                              'mustuse', 'dropins', 'core', and 'search'.
			 */
		...
		if ( ! $compatible_php || ! $compatible_wp ) {
			...
			if ( ! $compatible_php && ! $compatible_wp ) {
				$incompatible_message .= __( 'This plugin does not work with your versions of calmPress and PHP.' );
				if ( current_user_can( 'update_core' ) && current_user_can( 'update_php' ) ) {
					$incompatible_message .= sprintf(
						/* translators: 1: URL to calmPress Updates screen, 2: URL to Update PHP page. */
					...
					$incompatible_message .= sprintf(
						/* translators: %s: URL to calmPress Updates screen. */
				...
				$incompatible_message .= __( 'This plugin does not work with your version of calmPress.' );
