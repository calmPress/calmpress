	...
	protected function send_email( $type, $core_update, $result = null ) {
		...
		switch ( $type ) {
			case 'success': // We updated.
				/* translators: Site updated notification email subject. 1: Site title, 2: calmPress version. */
				$subject = __( '[%1$s] Your site has updated to calmPress %2$s' );
				break;

			case 'fail':   // We tried to update but couldn't.
			case 'manual': // We can't update (and made no attempt).
				/* translators: Update available notification email subject. 1: Site title, 2: calmPress version. */
		...
		switch ( $type ) {
			case 'success':
				$body .= sprintf(
					/* translators: 1: Home URL, 2: calmPress version. */
					__( 'Howdy! Your site at %1$s has been updated automatically to calmPress %2$s.' ),
					home_url(),
					$core_update->current
				);
				$body .= "\n\n";
				...
				if ( $newer_version_available ) {
					/* translators: %s: calmPress latest version. */
			...
			case 'manual':
				$body .= sprintf(
					/* translators: 1: Home URL, 2: calmPress version. */
					__( 'Please update your site at %1$s to calmPress %2$s.' ),
					home_url(),
			...
			case 'critical':
				if ( $newer_version_available ) {
					$body .= sprintf(
						/* translators: 1: Home URL, 2: calmPress version. */
						__( 'Your site at %1$s experienced a critical failure while trying to update calmPress to version %2$s.' ),
						home_url(),
						$core_update->current
					);
				} else {
					$body .= sprintf(
						/* translators: 1: Home URL, 2: calmPress latest version. */
						__( 'Your site at %1$s experienced a critical failure while trying to update to the latest version of calmPress, %2$s.' ),
						home_url(),
						$core_update->current
					);
				}
		...
		if ( $critical_support ) {
			...
			$body .= "\n\n" . sprintf(
				...
				__( 'The calmPress team is willing to help you. Forward this email to %s and the team will work with you to make sure your site is working.' ),
