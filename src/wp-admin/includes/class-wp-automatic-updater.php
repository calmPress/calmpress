	...
	protected function send_email( $type, $core_update, $result = null ) {
		...
		switch ( $type ) {
			case 'success': // We updated.
				/* translators: Site updated notification email subject. 1: Site name, 2: calmPress version number. */
				$subject = __( '[%1$s] Your site has updated to calmPress %2$s' );
				break;

			case 'fail':   // We tried to update but couldn't.
			case 'manual': // We can't update (and made no attempt).
				/* translators: Update available notification email subject. 1: Site name, 2: calmPress version number. */
		...
		switch ( $type ) {
			case 'success':
				...
				if ( $newer_version_available ) {
					/* translators: %s: calmPress core latest version */
			...
			case 'fail':
			case 'manual':
				/* translators: 1: Home URL, 2: calmPress core latest version */
	...
	protected function send_debug_email() {
		...
		if ( isset( $this->update_results['core'] ) ) {
			...
			if ( $result->result && ! is_wp_error( $result->result ) ) {
				/* translators: %s: calmPress core version */
