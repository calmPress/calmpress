 ...
 */
global $required_php_version, $required_php_extensions, $required_mysql_version, $wpdb;
...
if ( isset( $required_php_extensions ) && is_array( $required_php_extensions ) ) {
	foreach ( $required_php_extensions as $extension ) {
		...
		$missing_extensions[] = sprintf(
			...
			__( 'You cannot upgrade because <a href="%1$s">calmPress %2$s</a> requires the %3$s PHP extension.' ),
			$version_url,
			calmpress_version(),
			$extension
		);
...
<p id="logo"><?php _e( 'calmPress' ); ?></p>

<?php if ( (int) get_option( 'calmpress_db_version' ) === calmpress_version() || !is_blog_installed() ) : ?>
...
<p><?php _e( 'calmPress has been updated! Next and final step is to update your database to the newest version.' ); ?></p>
