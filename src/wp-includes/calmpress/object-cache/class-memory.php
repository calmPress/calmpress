<?php
/**
 * Implementation of memory storage for object caching.
 *
 * @package calmPress
 * @since 1.0.0
 */
...
/**
 * Implemantation ov memory based object cached storage. This cache will exists only
 * during the HTTP session and will be destroyed automatically when it ends.
 * 
 * There are no type hinting to be compatible with the interface defined in the PSR,
...
class Memory implements \Psr\SimpleCache\CacheInterface {
	...
	private static function throw_if_not_string( $value ) {
		if ( ! is_string( $value ) ) {
			throw new Invalid_Argument_Exception( 'value is not a string' );
		}
	}

	/**
	 * Helper that throws when a value is not iterable.
	 *
	 * @param mixed $value The value to validate.
	 *
	 * @throws Invalid_argument_Exception If $value is not string
	 */
	private static function throw_if_not_iterable( $value ) {
		if ( ! is_iterable( $value ) ) {
			throw new Invalid_Argument_Exception( 'value is not iterable' );
		}
	}
	...
	public function getMultiple( $keys, $default = null ) {
		...
		$ret = [];
		foreach ( $keys as $key ) {
			// get might throw if $key is not string.
	...
	public function setMultiple( $values, $ttl = null ) {
		static::throw_if_not_iterable( $values );

		foreach ( $values as $key => $value ) {
			// set might throw if $key is not string.
			$this->set( $key, $value );
		}
	...
	public function deleteMultiple( $keys ) {
		...
		foreach ( $keys as $key ) {
			// delete might throw if $key is not a string.
			$this->delete( $key );
		}
