...
function wp_authenticate_application_password(
	...
	if ( $error ) {
		// Internally pass a detailed error, but externaly just indicate a failure.
 ...
 *     @type int    $avatar_atachment_id  Optional, the ID of the attachment to be used as 
 *                                        avatar or 0 if user has no image based avatar.
...
function wp_get_password_hint() {
	$hint = __( 'You can use any password, but strong passwords are recommended: at least 12 characters, with a mix of letters, numbers, and symbols.' );
 ...
 *
 * @since 3.1.0
 * @since calmPress 1.0.0 login expected to be the user's email.
 *
...
function retrieve_password( $user_login = '' ) {
	...
	if ( empty( $user_login ) ) {
		$errors->add( 'empty_username', __( '<strong>Error</strong>: Please enter an email address.' ) );
	} else {
		$user_data = get_user_by( 'email', trim( wp_unslash( $user_login ) ) );
		if ( empty( $user_data ) ) {
			// Mask that nothing happens to prevent attackers from learning which emails exist in the system.
			return true;
		}
	}
	...
	if ( ! $user_data ) {
		// Mask the ability to learn which emails associated with a user.
		return true;
	}
	 ...
	 */
	$message .= network_site_url( 'wp-login.php?login=' . rawurlencode( $user_email ) . "&key=$key&action=rp", 'login' ) . '&wp_lang=' . $locale . "\r\n\r\n";
 ...
 *
 * In WordPress Application Passwords is supported only by sites using SSL or local environments.
 * On calmPress it is supported by default as non SSL sites are not supported and it is too much of a meh
 * if not supported on local sites.
 * 
 * Can be changed using the {@see 'wp_is_application_passwords_available'} filter.
 *
 * @since 5.9.0
 *
 * @return bool
