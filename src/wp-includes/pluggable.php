...
if ( ! function_exists( 'wp_authenticate' ) ) :
	...
	function wp_authenticate(
		...
		if ( null === $user || false === $user ) {
			 ...
			 */
			$user = new WP_Error( 'authentication_failed', __( '<strong>Error:</strong> Credencials given do not match any registered user.' ) );
		}
...
if ( ! function_exists( 'wp_redirect' ) ) :
	...
	function wp_redirect( $location, $status = 302, $x_redirect_by = 'WordPress' ) {
		...
		if ( 'cgi-fcgi' !== PHP_SAPI ) {
			status_header( $status ); // This causes problems on some FastCGI setups.
		}
...
if ( ! function_exists( 'wp_notify_postauthor' ) ) :
	/**
	 * Notifies an author (and/or others) of a comment on a post.
	 *
	 * @since 1.0.0
	 *
	...
	function wp_notify_postauthor( $comment_id, $deprecated = null ) {
		...
		$wp_email = 'calmpress@' . preg_replace( '#^www\.#', '', wp_parse_url( network_home_url(), PHP_URL_HOST ) );
...
if ( ! function_exists( 'wp_new_user_notification' ) ) :
	 ...
	 * @param string $notify     Optional. Type of notification that should happen. 'user' and 'both' notify the
	 *                           user. 'admin' and an empty string do not send a notification. Default empty.
	 */
...
if ( ! function_exists( 'wp_rand' ) ) :
	 ...
	 *
	 * @since 2.6.2
	 * @since 4.4.0 Uses PHP7 random_int().
...
if ( ! function_exists( 'get_avatar' ) ) :
	/**
	 * Retrieves the IMG tag of an avatar for a user, email address, comment, or post.
	 *
	 * @since 2.5.0
	 ...
	 * @param string $default    Ignored, kept for backward compatibility.
	 ...
	 *     @type bool         $force_default Ignored.
	 *     @type string       $rating        Ignored.
	 *     @type string       $scheme        URL scheme to use. See set_url_scheme() for accepted values.
	 *                                       Default null.
	 *     @type array|string $class         Array or string of additional classes to add to the img element.
	 *                                       Default null.
	 *     @type bool         $force_display Ignored.
	...
	function get_avatar( $id_or_email, $size = 96, $default_value = '', $alt = '', $args = null ) {
		 ...
		 * @param mixed       $id_or_email The Avatar to retrieve. Accepts a user_id, text,
		 *                                 user email, WP_User object, WP_Post object, or WP_Comment object.
