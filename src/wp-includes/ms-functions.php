...
/**
 * Activates a signup created through the WordPress self-registration flow.
 *
 * CalmPress retains the function so existing callers receive an error instead
 * of failing because the function does not exist.
 *
 * @since MU (3.0.0)
 * @since calmPress 1.0.0 Activation keys are not supported.
 * @deprecated calmPress 1.0.0 User invitations are activated when the user logs in.
 *
 * @param string $key Unused activation key retained for signature compatibility.
 * @return WP_Error Error explaining that invited users activate by logging in.
 */
function wpmu_activate_signup(
	...
	return new WP_Error(
		'activation_keys_not_supported',
		__( 'Activation keys are not supported. Invited users activate their account by logging in.' )
	);
}
...
function wpmu_welcome_notification(
	...
	if ( ! $welcome_email ) {
		/* translators: Do not translate SITE_NAME, BLOG_URL, PASSWORD: those are placeholders. */
		$welcome_email = __(
			'Howdy EMAIL,
...
function maybe_add_existing_user_to_blog() {
	...
	wp_die(
		sprintf(
			...
			__( 'You have been added to this site. Please visit the <a href="%1$s">homepage</a> or <a href="%2$s">log in</a> using your email and password.' ),
			home_url(),
			admin_url()
		),
		__( 'calmPress &rsaquo; Success' ),
		array( 'response' => 200 )
	);
}
...
function welcome_user_msg_filter( $text ) {
	if ( ! $text ) {
		...
		/* translators: Do not translate PASSWORD, LOGINLINK, SITE_NAME: those are placeholders. */
		$text = __(
			'Howdy EMAIL,
...
function update_network_option_new_admin_email( $old_value, $value ) {
	...
	/* translators: Do not translate DISPLAY_NAME, ADMIN_URL, EMAIL, SITENAME, SITEURL: those are placeholders. */
	$email_text = __(
		'Howdy ###DISPLAY_NAME###,
