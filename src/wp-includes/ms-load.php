...
function ms_not_installed( $domain, $path ) {
	...
	if ( ! $wpdb->get_var( $query ) ) {
		$msg .= '<p>' . sprintf(
			...
			__( '<strong>Database tables are missing.</strong> This means that your host&#8217;s database server is not running, calmPress was not installed properly, or someone deleted %s. You really should look at your database now.' ),
			'<code>' . $wpdb->site . '</code>'
		) . '</p>';
	} else {
		$msg .= '<p>' . sprintf(
