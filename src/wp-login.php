 ...
 *
 * @param string   $title    Optional. calmPress login Page title to display in the `<title>` element.
 *                           Default 'Log In'.
...
default:
	...
	if ( empty( $_COOKIE[ LOGGED_IN_COOKIE ] ) ) {
		if ( headers_sent() ) {
			...
			$user = new WP_Error( 'test_cookie', sprintf( __( '<strong>ERROR</strong>: Cookies are blocked or not supported by your browser. You must <a href="%s">enable cookies</a> to use calmPress.' ),
...
<?php		exit;
	...
	if ( $interim_login ) {
		...
		elseif ( strpos( $redirect_to, 'about.php?updated' ) )
			$errors->add('updated', __( '<strong>You have successfully updated calmPress!</strong> Please log back in to see what&#8217;s new.' ), 'message' );
	}
