...
class Tests_General_DocumentTitle extends WP_UnitTestCase {
	...
	function test_front_page_title() {
		...
		$this->go_to( '/' );
		$this->assertEquals( sprintf( '%s &#8211; Just another calmPress site', $this->blog_name ), wp_get_document_title() );
	...
	function test_paged_title() {
		...
		$this->assertEquals( sprintf( '%s &#8211; Page 4 &#8211; Just another calmPress site', $this->blog_name ), wp_get_document_title() );
	}
