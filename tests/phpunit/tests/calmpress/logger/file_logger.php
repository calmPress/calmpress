...
class File_Logger_Test extends WP_UnitTestCase {
	...
	function test_constructor() {
		...
		$logger = new \calmpress\logger\File_Logger( WP_CONTENT_DIR . '/.private/logs/test' );
	...
	function test_log_message() {
		...
		$this->assertTrue( is_file( WP_CONTENT_DIR . '/.private/logs/test/' . $date . '.log' ) );
		...
		$filesize = filesize( WP_CONTENT_DIR . '/.private/logs/test/' . $date . '.log' );
		...
		clearstatcache();
		$filesize2 = filesize( WP_CONTENT_DIR . '/.private/logs/test/' . $date . '.log' );
	...
	public function test_purge_old_log_entries() {
		...
		file_put_contents( WP_CONTENT_DIR . '/.private/logs/test/2023-10-3.log', '' );

		// old file to remove
		file_put_contents( WP_CONTENT_DIR . '/.private/logs/test/2023-09-3.log', '' );
		touch( WP_CONTENT_DIR . '/.private/logs/test/2023-09-3.log', time() - 2 * DAY_IN_SECONDS );

		$logger->purge_old_log_entries( 1 );
		$this->assertTrue( file_exists( WP_CONTENT_DIR . '/.private/logs/test/2023-10-3.log' ) );
		$this->assertFalse( file_exists( WP_CONTENT_DIR . '/.private/logs/test/2023-09-3.log' ) );
	}
}