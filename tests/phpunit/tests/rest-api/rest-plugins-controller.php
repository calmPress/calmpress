...
class WP_REST_Plugins_Controller_Test extends WP_Test_REST_Controller_Testcase {
	...
	const NETWORK_PLUGIN      = 'network-test-plugin/network-test-plugin';
	const NETWORK_PLUGIN_FILE = self::NETWORK_PLUGIN . '.php';
	...
	public function test_get_items_excludes_network_only_plugin_if_not_active() {
		...
		$items = wp_list_filter( $response->get_data(), array( 'plugin' => self::NETWORK_PLUGIN ) );
	...
	public function test_get_item_invalid_plugin() {
		wp_set_current_user( self::$super_admin );
		$response = rest_do_request( self::BASE . '/' . self::PLUGIN .'y' );
	...
	public function test_update_item_activate_network_only_plugin() {
		...
		$request = new WP_REST_Request( 'PUT', self::BASE . '/' . self::NETWORK_PLUGIN );
		$request->set_body_params( array( 'status' => 'active' ) );
		$response = rest_do_request( $request );
	...
	/**
	 * As the test plugin file is in git it is impossible to delete it.
	 */
	public function test_delete_item() {
		$this->MarkTestSkipped( 'Test plugin directory is read only, therefor plugins can not be deleted.');
	}
	...
	public function test_prepare_item_network_only() {
		...
		$item          = get_plugins()[ self::NETWORK_PLUGIN_FILE ];
		$item['_file'] = self::NETWORK_PLUGIN_FILE;

		$endpoint = new WP_REST_Plugins_Controller();
		$response = $endpoint->prepare_item_for_response( $item, new WP_REST_Request( 'GET', self::BASE . '/' . self::NETWORK_PLUGIN ) );
	...
	protected function check_get_plugin_data( $data, $network_only = false ) {
		if ( $network_only ) {
			$this->assertSame( 'network-test-plugin/network-test-plugin', $data['plugin'] );
		} else {
